...
⚠️ **互動元件確認規定：**
- 在呈現簡報大綱草稿時，**必須主動向使用者詢問是否需要使用 Firebase 互動元件（如即時文字雲 [INTERACT:wordcloud] 或單選投票 [INTERACT:poll]）**。
- 如果使用者回覆不需要，則在大綱與後續生成中，將對應頁面改為靜態提問或討論頁，並在 HTML 中排除 Firebase 連線腳本與配置，以簡化簡報結構與避免不必要的配置。
